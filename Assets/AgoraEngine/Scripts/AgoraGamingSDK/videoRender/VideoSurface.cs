    ...
    */
    ...
    public class VideoSurface : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            {
                NetworkedUintManager[] managers = FindObjectsOfType<NetworkedUintManager>();
                NetworkedUintManager matchingManager = null;
                foreach (NetworkedUintManager manager in managers)
                {
                    if (manager.outputUint.ToString() == transform.name)
                    {
                ...
                {
                    transform.SetParent(matchingManager.agoraVideoChat.spawnPoint);
                    transform.localPosition = new Vector3(0, 0, 0);
                }
            }
        }
